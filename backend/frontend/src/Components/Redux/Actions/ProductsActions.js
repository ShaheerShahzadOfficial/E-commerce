...
export const GetProduct = (keyword = "", CurrentPage = 1, Price = [1000, 25000], Category) => async (dispatch) => {
    ...
    let link = `/products/getProduct?keyword=${keyword}&page=${CurrentPage}&price[gte]=${Price[0]}&price[lte]=${Price[1]}`

    if (Category) {
        link = `/products/getProduct?keyword=${keyword}&page=${CurrentPage}&price[gte]=${Price[0]}&price[lte]=${Price[1]}&category=${Category}`
    }
...
export const GetProductDetail = (id) => async (dispatch) => {
    ...
    await axios.get(`/products/ProductDetail/${id}`).then((result) => {
        dispatch({
...
export const AddReviews = (productId, rating, comment) => async (dispatch) => {
    ...
    await axios.put("/products/addProductReview", {
        productId,
        rating,
        comment
...
export const GetAdminProduct = () => async (dispatch) => {
    ...
    await axios.get("/products/admin/getProduct",
...
export const deleteProduct = (id) => async (dispatch) => {
    ...
    await axios.delete(`/products/deleteProduct/${id}`).then((result) => {
        dispatch({
...
export const updateProduct = (id, name, price, description, category, Stock, images) => async (dispatch) => {
    ...
    await axios.put(`/products/updateProduct/${id}`, {
